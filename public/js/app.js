    ...
    async updateBids() {
        try {
            ...
            if (data.success) {
                ...
                // 추가 정보가 있으면 토스트로 표시
                if (data.newBids > 0 || data.removedSelected > 0 || data.updatedSelected > 0) {
                    let details = [];
                    if (data.newBids > 0) {
                        details.push(`새로운 공고: ${data.newBids}개`);
                    }
                    if (data.removedSelected > 0) {
                        details.push(`선택 목록에서 제거: ${data.removedSelected}개`);
                    }
                    if (data.updatedSelected > 0) {
